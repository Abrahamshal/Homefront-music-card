...
var dispatchPrefix = "hmc-dispatch-event-";
var ACTIVE_PLAYER_EVENT_INTERNAL = "active-player";
var ACTIVE_PLAYER_EVENT = "hmc-dispatch-event-active-player";
...
var SESSION_STORAGE_PLAYER_ID = "hmc-player-id";
...
customElements.define("hmc-icon-button", IconButton);
//#endregion
...
customElements.define("hmc-player-favorite-button", PlayerFavoriteButton);
//#endregion
	...
	render() {
      ...
      <div class="main" id="mediaControls" style=${(controlsColor ? `--controls-color: ${controlsColor}` : "") || E}>
        <div class="icons ${controlsLargeIcons ? "large-icons" : ""}">
          <div class="flex-1">
            <hmc-player-favorite-button .store=${this.store}></hmc-player-favorite-button>
          </div>
          <hmc-icon-button ?hidden=${!showVolumeUpAndDownButtons} @click=${this.volDown} .path=${mdiVolumeMinus}></hmc-icon-button>
          <hmc-shuffle ?hidden=${!!hideControlShuffleButton} .store=${this.store}></hmc-shuffle>
          <hmc-icon-button ?hidden=${!!hideControlPrevTrackButton} @click=${this.prev} .path=${mdiSkipPrevious}></hmc-icon-button>
          <hmc-icon-button ?hidden=${!showFastForwardAndRewindButtons} @click=${this.rewind} .path=${mdiRewind}></hmc-icon-button>
          <hmc-icon-button @click=${playPauseHandler} .path=${playPauseIcon} class="big-icon"></hmc-icon-button>
          <hmc-icon-button ?hidden=${!showFastForwardAndRewindButtons} @click=${this.fastForward} .path=${mdiFastForward}></hmc-icon-button>
          <hmc-icon-button ?hidden=${!!hideControlNextTrackButton} @click=${this.next} .path=${mdiSkipNext}></hmc-icon-button>
          <hmc-repeat ?hidden=${!!hideControlRepeatButton} .store=${this.store}></hmc-repeat>
          <hmc-icon-button ?hidden=${!showVolumeUpAndDownButtons} @click=${this.volUp} .path=${mdiVolumePlus}></hmc-icon-button>
          <div class="flex-1">
            <hmc-icon-button
              class="browse-button"
        ...
        ></hmc-volume>
        <div class="icons">
          <hmc-icon-button ?hidden=${hidePower} @click=${this.togglePower}></hmc-icon-button>
        </div>
      </div>
    `;
	}
...
customElements.define("hmc-player-controls", PlayerControls);
//#endregion
...
var PlayerHeader = class extends i$5 {
	render() {
      ...
      <hmc-progress .store=${this.store}></hmc-progress>
    </div>`;
	}
	getSong() {
	...
	static get styles() {
      ...
      .entity {
        overflow: hidden;
        text-overflow: ellipsis;
        font-size: var(--hmc-font-size, 1rem);
        font-weight: 500;
        color: var(--secondary-text-color);
        white-space: nowrap;
      }
      ...
      .song {
        overflow: hidden;
        text-overflow: ellipsis;
        font-size: calc(var(--hmc-font-size, 1rem) * 1.15);
        font-weight: 400;
        color: var(--accent-color);
      }
...
customElements.define("hmc-player-header", PlayerHeader);
//#endregion
...
var Volume = class extends i$5 {
	...
	render() {
      ...
      <div class="volume" slim=${this.slim || E}>
        ...
        <hmc-icon-button hide=${this.store.hidePower()} @click=${this.togglePower} .path=${mdiPower} style=${buttonStyle}></hmc-icon-button>
      </div>
    `;
	}
	getMax() {
	...
	static get styles() {
      ...
      *[slim] .volume-slider {
      ...
      .volume-level {
        font-size: calc(var(--hmc-font-size, 1rem) * 0.75);
        display: flex;
      }
...
var Player = class extends i$5 {
	...
	render() {
      ...
      <div class="container ${blurAmount > 0 ? "blurred-background" : ""}" style=${containerStyle || E}>
        <hmc-player-header class="header" background=${artworkAsBackground || E} .store=${this.store} ?hidden=${!!hideHeader}></hmc-player-header>
...
var MusicAssistantService = class {
	...
	*/
	...
	/**
	* Browse Music Assistant's native source/account tree via the
	* `music/browse` command — the same API MA's own "Browse" tab uses.
	*
	* - `path` omitted / null → root: one folder per provider/account
	*   (e.g. "Abe's Spotify" → `spotify--Umazu89E://`, "Marlene's Spotify"
	*   → `spotify--BEPYczcm://`, "Eileen's Pandora" → `pandora://`).
	* - `path` set → that node's children (sub-folders, then playable items).
	*
	* This is the per-account separation that HA's merged
	* `media_player/browse_media` cannot provide.
	*/
	async browse(massQueueConfigEntryId, path) {
...
var MediaBrowseService = class {
	constructor(hass, config) {
		...
		this.massQueueConfigEntryId = null;
		this.massQueueDiscoveryDone = false;
		this.hass = hass;
		this.config = config;
		this.musicAssistantService = new MusicAssistantService(hass);
	}
	async getMassQueueConfigEntryId() {
...
var Store = class {
	...
	getActivePlayerFromStorage() {
		return window.sessionStorage.getItem("hmc-player-id") || "";
	}
...
customElements.define("hmc-section-button", SectionButton);
//#endregion
...
var Footer = class extends i$5 {
	render() {
      ...
      ${sections.map(([section, icon]) => x`
          <hmc-section-button .config=${this.config} .icon=${icon} .selectedSection=${this.section} .section=${section}></hmc-section-button>
        `)}
    `;
	}
	static get styles() {
...
customElements.define("hmc-footer", Footer);
//#endregion
...
customElements.define("hmc-predefined-group-editor", PredefinedGroupEditor);
//#endregion
...
customElements.define("hmc-common-tab", CommonTab);
//#endregion
...
customElements.define("hmc-artwork-override-editor", ArtworkOverrideEditor);
//#endregion
...
customElements.define("hmc-player-tab", PlayerTab);
//#endregion
...
customElements.define("hmc-media-browser-tab", MediaBrowserTab);
//#endregion
...
customElements.define("hmc-section-tab", SectionTab);
//#endregion
...
customElements.define("hmc-editor-form", Form);
//#endregion
...
var CardEditor = class extends BaseEditor {
	...
	render() {
      ...
      <div class="tabs-container">
        <hmc-icon-button class="nav-arrow ${showLeftArrow ? "" : "hidden"}" .path=${mdiChevronLeft} @click=${this.navigatePrev}></hmc-icon-button>
        <div class="tabs-list">
          ${tabs.map((tab) => x` <button class="tab-button ${this.activeTab === tab ? "active" : ""}" @click=${() => this.activeTab = tab}>${tab}</button> `)}
        </div>
        <hmc-icon-button class="nav-arrow ${showRightArrow ? "" : "hidden"}" .path=${mdiChevronRight} @click=${this.navigateNext}></hmc-icon-button>
      </div>
	...
	renderTabContent() {
		...
		const t = (s, sec) => x`<hmc-section-tab .schema=${s} .section=${sec} .config=${c} .hass=${h}></hmc-section-tab>`;
		return r(this.activeTab, [
			["Common", () => x`<hmc-common-tab .config=${c} .hass=${h}></hmc-common-tab>`],
			["Player", () => x`<hmc-player-tab .config=${c} .hass=${h}></hmc-player-tab>`],
			["Media Browser", () => x`<hmc-media-browser-tab .config=${c} .hass=${h}></hmc-media-browser-tab>`],
...
customElements.define("hmc-editor", CardEditor);
//#endregion
...
var Shuffle = class extends i$5 {
	...
	render() {
		...
		this.mediaControlService = this.store.mediaControlService;
		return x`<hmc-icon-button @click=${this.shuffle} .path=${this.shuffleIcon()}></hmc-icon-button> `;
	}
	shuffleIcon() {
...
customElements.define("hmc-shuffle", Shuffle);
//#endregion
...
var Repeat = class extends i$5 {
	...
	render() {
		this.activePlayer = this.store.activePlayer;
		this.mediaControlService = this.store.mediaControlService;
		return x`<hmc-icon-button @click=${this.repeat} .path=${this.repeatIcon()}></hmc-icon-button> `;
	}
	repeatIcon() {
...
customElements.define("hmc-repeat", Repeat);
//#endregion
...
customElements.define("hmc-source", Source);
//#endregion
//#region src/card.ts
...
var Card = class extends i$5 {
      ...
      <ha-card style=${this.haCardStyle(height)}>
        ...
        <div class="content" style=${this.contentStyle(contentHeight)}>
          ${this.activePlayerId ? r(this.section, [
			[PLAYER, () => x` <hmc-player .store=${this.store}></hmc-player>`],
			[GROUPS, () => x` <hmc-groups .store=${this.store} @active-player=${this.activePlayerListener}></hmc-groups>`],
			[GROUPING, () => x`<hmc-grouping .store=${this.store} @active-player=${this.activePlayerListener}></hmc-grouping>`],
			[VOLUMES, () => x` <hmc-volumes .store=${this.store}></hmc-volumes>`],
			[MEDIA_BROWSER, () => x`<hmc-media-browser .store=${this.store} @item-selected=${this.onMediaItemSelected}></hmc-media-browser>`],
			[QUEUE, () => x`<hmc-queue .store=${this.store} @item-selected=${this.onMediaItemSelected}></hmc-queue>`],
			[SEARCH, () => x`<hmc-search .store=${this.store} @item-selected=${this.onMediaItemSelected}></hmc-search>`]
		]) : x`<div class="no-players">${noPlayersText}</div>`}
        </div>
        ${n(showFooter, () => x`<hmc-footer
	...
	static getConfigElement() {
		return document.createElement("hmc-editor");
	}
	connectedCallback() {
	...
	haCardStyle(height) {
		...
		return o$1({
			...
			...this.config.baseFontSize ? {
				...
				"--hmc-font-size": `${this.config.baseFontSize}rem`,
	...
	static get styles() {
      ...
      .title {
        ...
        text-align: center;
        font-weight: bold;
        font-size: calc(var(--hmc-font-size, 1rem) * 1.2);
        color: var(--secondary-text-color);
      }
      .no-players {
        text-align: center;
...
customElements.define("hmc-grouping-button", GroupingButton);
//#endregion
...
customElements.define("hmc-grouping-actions", GroupingActions);
//#endregion
...
customElements.define("hmc-grouping-item-row", GroupingItemRow);
//#endregion
...
var Grouping = class extends i$5 {
	...
	render() {
      ...
      <div class="wrapper">
        ...
        <div class="list">
          ${items.map((item) => x`
              <hmc-grouping-item-row .store=${store} .item=${item} .applying=${applying} @toggle-item=${this.handleToggleItem}></hmc-grouping-item-row>
            `)}
        </div>
...
customElements.define("hmc-playing-bars", PlayingBars);
//#endregion
...
customElements.define("hmc-group-icons", GroupIcons);
//#endregion
...
var Group = class extends i$5 {
	...
	render() {
      ...
      >
        <div class="row">
          <hmc-group-icons .icons=${icons}></hmc-group-icons>
          <div class="text">
        ...
        <hmc-playing-bars slot="meta" .show=${this.player.isPlaying()}></hmc-playing-bars>
      </mwc-list-item>
    `;
	}
	connectedCallback() {
	...
	static get styles() {
      ...
      .speakers {
        white-space: initial;
        font-size: calc(var(--hmc-font-size, 1rem) * 1.1);
        font-weight: bold;
        color: var(--secondary-text-color);
      }

      .song-title {
        font-size: calc(var(--hmc-font-size, 1rem) * 0.9);
        font-weight: bold;
      }
...
customElements.define("hmc-group", Group);
//#endregion
	...
	render() {
      ...
      <mwc-list activatable class="list" style=${buttonWidth ? o$1({ width: `${buttonWidth}rem` }) : ""}>
        ${this.store.allGroups.map((group) => {
			...
			return x` <hmc-group .store=${this.store} .player=${group} .selected=${selected}></hmc-group> `;
		})}
      </mwc-list>
    `;
	}
	...
	render() {
      ...
      <mwc-list multi class="list">
        ${itemsWithFallbacks(this.items, this.config).map((item) => {
			return x`
            <hmc-media-row @click=${() => this.dispatchEvent(customEvent(MEDIA_ITEM_SELECTED, item))} .item=${item} .store=${this.store}></hmc-media-row>
          `;
		})}
      </mwc-list>
    `;
...
customElements.define("hmc-favorites-list", FavoritesList);
//#endregion
...
var mediaGridCardStyles = i$8`
  ...
  .child .title {
    color: var(--secondary-text-color);
    font-size: calc(var(--hmc-font-size, 1rem) * 0.8);
    font-weight: normal;
    padding-top: 7px;
    padding-inline: 8px;
    overflow: hidden;
...
function renderShortcutButton(shortcut, onClick, isActive = false) {
	...
	return x`
    <hmc-icon-button class=${isActive ? "shortcut-active" : ""} @click=${onClick} title=${shortcut.name} .path=${icon.startsWith("mdi:") ? void 0 : icon}>
...
customElements.define("hmc-favorites-icons", FavoritesIcons);
//#endregion
...
var Favorites = class Favorites extends i$5 {
	...
	render() {
        ...
        <hmc-favorites-icons .items=${this.cachedFavorites} .store=${this.store} @item-selected=${this.onFavoriteSelected}></hmc-favorites-icons>
      `;
		else return x`
        <hmc-favorites-list .items=${this.cachedFavorites} .store=${this.store} @item-selected=${this.onFavoriteSelected}></hmc-favorites-list>
      `;
	}
	static get styles() {
		return i$8`
...
HaMediaPlayerBrowse = __decorate$1([t$3("hmc-ha-media-player-browse")], HaMediaPlayerBrowse);
//#endregion
...
var mediaBrowserStyles = i$8`
  ...
  hmc-icon-button.startpath-active {
    color: var(--accent-color);
  }
  hmc-icon-button.shortcut-active {
    color: var(--accent-color);
  }
  .play-all-warning {
    padding: 8px 12px;
...
function renderLayoutMenu(layout, onSelect) {
    ...
    <ha-dropdown @wa-select=${onSelect}>
      <hmc-icon-button slot="trigger" .path=${mdiDotsVertical}></hmc-icon-button>
...
var START_PATH_KEY$1 = "hmc-media-browser-start";
var MediaBrowserBrowser = class extends i$5 {
	...
	render() {
      ...
      ${config.hideHeader ? "" : x`<div class="header">
            ${this.navigateIds.length > 1 ? x`<hmc-icon-button .path=${mdiArrowLeft} @click=${this.goBack}></hmc-icon-button>` : x`<div class="spacer"></div>`}
      ...
      ${i(this.layout, x`<hmc-ha-media-player-browse
	...
	renderPlayAllButton() {
		...
		return x`<hmc-icon-button .path=${mdiPlay} @click=${this.handlePlayAll} title="Play all (${playableCount} tracks)"></hmc-icon-button>`;
	}
	async handlePlayAll() {
...
__decorate$1([e$3("hmc-ha-media-player-browse")], MediaBrowserBrowser.prototype, "mediaBrowser", void 0);
//#endregion
//#region src/sections/media-browser/sources/sources.ts
var currentNav = null;
/**
...
var MediaBrowserSources = class extends i$5 {
	constructor(..._args) {
		super(..._args);
		this.layout = "auto";
		this.nav = currentNav ?? [{ title: "Sources" }];
		this.items = [];
		this.loading = false;
		this.error = "";
	...
	render() {
		...
		return x`
      ${config.hideHeader ? "" : x`<div class="header">
            ${this.nav.length > 1 ? x`<hmc-icon-button .path=${mdiArrowLeft} @click=${this.goBack}></hmc-icon-button>` : x`<div class="spacer"></div>`}
            <div class="title-section">
...
__decorate$1([n$5({ type: String })], MediaBrowserSources.prototype, "layout", void 0);
__decorate$1([r$3()], MediaBrowserSources.prototype, "nav", void 0);
...
var START_PATH_KEY = "hmc-media-browser-start";
var LAYOUT_KEY = "hmc-media-browser-layout";
...
var MediaBrowser = class extends i$5 {
	...
	render() {
		...
		if (this.view === "sources") return this.renderSources();
		return this.renderFavorites();
	}
	renderSources() {
		return x`
      <hmc-media-browser-sources
        .store=${this.store}
        .layout=${this.layout}
	...
	renderFavorites() {
      ...
      ${config.hideHeader ? "" : x`<div class="header">
            ...
            ${onlyFavorites ? "" : x`<hmc-icon-button .path=${mdiAccountMusic} @click=${this.goToSources} title="Browse by source/account"></hmc-icon-button>
                  <hmc-icon-button .path=${mdiPlayBoxMultiple} @click=${this.goToBrowser} title="Browse Media"></hmc-icon-button>
                  <hmc-icon-button
                    class=${this.isCurrentPathStart ? "startpath-active" : ""}
                    .path=${this.isCurrentPathStart ? mdiFolderStar : mdiFolderStarOutline}
                    @click=${this.toggleStartPath}
      ...
      <hmc-favorites .store=${this.store} .layout=${this.layout} @item-selected=${this.onMediaItemSelected}></hmc-favorites>
    `;
	}
	renderBrowser() {
		return x`
      <hmc-media-browser-browser
        .store=${this.store}
        .layout=${this.layout}
        @item-selected=${this.onMediaItemSelected}
        @go-to-favorites=${this.goToFavorites}
...
__decorate$1([e$3("hmc-media-browser-browser")], MediaBrowser.prototype, "browserComponent", void 0);
//#endregion
...
customElements.define("hmc-operation-overlay", OperationOverlay);
//#endregion
...
var PlayMenu = class extends i$5 {
	...
	renderButtonMenu() {
      ...
      <ha-dropdown @wa-select=${this.handleAction}>
        <hmc-icon-button slot="trigger" .path=${mdiPlay} title="Play options" ?disabled=${this.disabled}></hmc-icon-button>
	...
	renderInlineMenu() {
      ...
      <div class="inline-menu" @click=${(e) => e.stopPropagation()}>
        <hmc-icon-button class="close-btn" .path=${mdiClose} @click=${this.closeMenu} title="Close"></hmc-icon-button>
...
customElements.define("hmc-play-menu", PlayMenu);
//#endregion
//#region src/sections/queue/queue-search-utils.ts
var QUEUE_SEARCH_STORAGE_KEY = "hmc-queue-search-state";
...
var queueSearchStyles = i$8`
  ...
  .search-row hmc-icon-button {
    --icon-button-size: 2rem;
    --icon-size: 1.2rem;
  }
  .search-row hmc-icon-button[selected] {
    color: var(--accent-color);
  }
`;
//#endregion
...
var QueueSearchPanel = class extends i$5 {
	...
	render() {
      ...
      <div class="search-row">
        ...
        <hmc-icon-button .path=${mdiChevronUp} @click=${() => this.dispatchAction({ type: "prev" })} ?hidden=${!hasMatches}></hmc-icon-button>
        <hmc-icon-button .path=${mdiChevronDown} @click=${() => this.dispatchAction({ type: "next" })} ?hidden=${!hasMatches}></hmc-icon-button>
        <hmc-icon-button
          .path=${mdiCheckAll}
...
customElements.define("hmc-queue-search-panel", QueueSearchPanel);
//#endregion
...
__decorate$1([e$3("hmc-queue-search-panel")], QueueSearch.prototype, "panel", void 0);
customElements.define("hmc-queue-search", QueueSearch);
//#endregion
...
var SelectionActions = class extends i$5 {
	...
	render() {
		return x`
      ${this.showInvert ? x`<hmc-icon-button .path=${mdiSelectInverse} @click=${this.invertSelection} title="Invert selection"></hmc-icon-button>` : E}
      <hmc-play-menu .hasSelection=${this.hasSelection} .disabled=${this.disabled} @play-menu-action=${this.onPlayMenuAction}></hmc-play-menu>
    `;
	}
	invertSelection() {
...
customElements.define("hmc-selection-actions", SelectionActions);
//#endregion
	...
	render() {
      ...
      <div class="header">
        ...
        <div class="header-icons">
          <hmc-queue-search .items=${this.items} .selectMode=${this.selectMode} @queue-search-action=${this.onSearchAction}></hmc-queue-search>
          <div ?hidden=${!this.selectMode}>
            <hmc-selection-actions
            ...
            <div class="delete-all-btn" @click=${() => this.dispatchAction({ type: "clear-queue" })} title="Delete all">
              <hmc-icon-button .path=${mdiTrashCanOutline}></hmc-icon-button>
              <span class="all-label">*</span>
            </div>
          </div>
          <div ?hidden=${this.selectMode}>
            <hmc-shuffle .store=${this.store}></hmc-shuffle>
            <hmc-repeat .store=${this.store}></hmc-repeat>
          </div>
          <hmc-icon-button
...
_QueueHeader.styles = i$8`
    ...
    .player-name {
      font-size: calc(var(--hmc-font-size, 1rem) * 0.85);
      color: var(--secondary-text-color);
      overflow: hidden;
      text-overflow: ellipsis;
      white-space: nowrap;
...
customElements.define("hmc-queue-header", QueueHeader);
//#endregion
	...
	render() {
      ...
      >
        ...
        <div class="meta-content" slot="meta">
          <hmc-playing-bars .show=${this.playing}></hmc-playing-bars>
...
customElements.define("hmc-media-row", MediaRow);
//#endregion
...
var QueueList = class extends i$5 {
	...
	scrollToItem(index) {
		this.shadowRoot?.querySelectorAll("hmc-media-row")?.[index]?.scrollIntoView({
			behavior: "smooth",
			block: "center"
		});
	}
...
customElements.define("hmc-queue-list", QueueList);
//#endregion
...
var QueueController = class {
	...
	async scrollToCurrentlyPlaying() {
		...
		(this.host.shadowRoot?.querySelectorAll("hmc-media-row")[this.selectedQueueIndex])?.scrollIntoView({
			behavior: "smooth",
			block: "center"
		});
	}
...
//#endregion
//#region src/sections/queue/queue-hmc-utils.ts
...
var QueueSonos = class extends i$5 {
	...
	async refetchAndScroll() {
		...
		const pos = this.store.activePlayer.attributes.queue_position;
		if (pos) this.shadowRoot?.querySelector("hmc-queue-list")?.scrollToItem(pos - 1);
	}
...
var Queue = class extends i$5 {
	render() {
		return this.store.config.entityPlatform === "music_assistant" ? x`<hmc-queue-mass .store=${this.store}></hmc-queue-mass>` : x`<hmc-queue-sonos .store=${this.store}></hmc-queue-sonos>`;
	}
};
_Queue = Queue;
_Queue.styles = i$8`
...
var LOCAL_STORAGE_KEY = "hmc-search-state";
...
var searchStyles = [sectionCommonStyles, i$8`
    ...
    .media-type-icons hmc-icon-button[selected] {
      color: var(--accent-color);
    }
    .separator {
      width: 1px;
    ...
    .library-filter-btn hmc-icon-button[selected] {
      color: var(--accent-color);
    }
    ...
    .filter-menu-anchor hmc-icon-button[selected] {
      color: var(--accent-color);
    }
    .filter-menu {
      position: absolute;
...
customElements.define("hmc-search-filter-menu", SearchFilterMenu);
//#endregion
...
customElements.define("hmc-search-header", SearchHeader);
//#endregion
...
var SearchBar = class extends i$5 {
	...
	render() {
      ...
      <div class="search-bar">
        <hmc-icon-button .path=${mdiMagnify} @click=${() => this.dispatchEvent(customEvent("search-submit"))}></hmc-icon-button>
...
customElements.define("hmc-search-bar", SearchBar);
//#endregion
...
customElements.define("hmc-search-results", SearchResults);
//#endregion
...
var SearchBrowseView = class extends i$5 {
	...
	render() {
      ...
      <div class="header browse-header">
        <hmc-icon-button .path=${mdiArrowLeft} @click=${this.goBack} title="Back to results"></hmc-icon-button>
        <ha-dropdown @wa-select=${this.handlePlayMenuAction}>
          <hmc-icon-button slot="trigger" .path=${mdiPlay} title="Play options"></hmc-icon-button>
          <ha-dropdown-item value="0">
            <ha-svg-icon slot="icon" .path=${mdiPlayBoxMultiple}></ha-svg-icon>
            Play Now (clear queue)
          </ha-dropdown-item>
        ...
        <hmc-icon-button .path=${typeIcon} disabled class="type-indicator"></hmc-icon-button>
      </div>
      ...
      <div class="list" ?hidden=${this.browseResults.length === 0}>
        <mwc-list multi>
          ${c(this.browseResults, (browseItem) => browseItem.uri, (browseItem, index) => {
			return x` <hmc-media-row @click=${() => this.onBrowseItemClick(index)} .item=${toMediaPlayerItem(browseItem)} .store=${this.store}></hmc-media-row> `;
		})}
        </mwc-list>
      </div>
    `;
...
customElements.define("hmc-search-browse-view", SearchBrowseView);
//#endregion
...
__decorate$1([e$3("hmc-search-results")], Search.prototype, "searchResults", void 0);
__decorate$1([e$3("hmc-search-bar")], Search.prototype, "searchBar", void 0);
//#endregion
...
var SleepTimer = class extends i$5 {
	render() {
		...
		return x`
      <div id="sleepTimer">
        <hmc-icon-button id="sleepTimerAlarm" .path=${mdiAlarm}></hmc-icon-button>
        ...
        ></hmc-icon-button>
        <hmc-icon-button id="sleepTimerCancel" .path=${mdiCloseCircle} @click=${() => hassService.cancelSleepTimer(this.player)}></hmc-icon-button>
      </div>
    `;
	}
	static get styles() {
...
customElements.define("hmc-sleep-timer", SleepTimer);
//#endregion
	...
	volumeWithName(player, updateMembers = true, groupName = "") {
      ...
      <div class="slider-row">
        <hmc-icon-button
          ...
          style=${buttonStyle}
        ></hmc-icon-button>
        <hmc-volume .store=${this.store} .player=${player} .updateMembers=${updateMembers}></hmc-volume>
        <hmc-icon-button
          .disabled=${player.ignoreVolume}
          ?hidden=${!showVolumeUpAndDownButtons}
          @click=${volUp}
      ...
      <div class="switches" ?hidden=${hideSwitches}>
        <hmc-source .store=${this.store} .player=${player}> </hmc-source>
        ${m(this.getAdditionalControls(hideSwitches, player))}
        <hmc-sleep-timer .store=${this.store} .player=${player}></hmc-sleep-timer>
      </div>
    </div>`;
	}
...
customElements.define("homefront-music-card", Card);
customElements.define("hmc-grouping", Grouping);
customElements.define("hmc-groups", Groups);
customElements.define("hmc-media-browser", MediaBrowser);
customElements.define("hmc-media-browser-browser", MediaBrowserBrowser);
customElements.define("hmc-favorites", Favorites);
customElements.define("hmc-player", Player);
customElements.define("hmc-volumes", Volumes);
customElements.define("hmc-queue", Queue);
customElements.define("hmc-queue-mass", QueueMass);
customElements.define("hmc-queue-sonos", QueueSonos);
customElements.define("hmc-search", Search);
//#endregion

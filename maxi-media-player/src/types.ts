...
/**
 * Raw item returned by Music Assistant's native `music/browse` command
 * (the API its own UI uses for the source/account-separated Browse tab).
...
export interface MassBrowseItem {
  ...
  name: string; // human name for accounts/leaves; EMPTY for category folders (use translation_key)
  uri: string;

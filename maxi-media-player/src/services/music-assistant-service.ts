...
export class MusicAssistantService {
   ...
   */
  async browse(massQueueConfigEntryId: string, path?: string): Promise<MediaPlayerItem[]> {
    const data: Record<string, unknown> = path ? { path } : {};
    const items = await this.sendMassCommand<MassBrowseItem[]>(massQueueConfigEntryId, 'music/browse', data, true);
